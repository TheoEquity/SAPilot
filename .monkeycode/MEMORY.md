...
[SAPilot 稳定版本变更纪律]
- Date: 2026-05-26
- Context: 用户强调远程仓库已验证版本为稳定基线，后续改动需先充分沟通
- Instructions:
  - 对已验证稳定的链路，先分析原因，再结合用户提供的背景充分沟通。
  - 只有在与用户确认根因后，才执行代码修改。
  - 不要基于主观判断直接修改代码，避免问题叠加和现场变乱。
...
[SAPilot 钉钉附件代码分析范围]
- Date: 2026-05-25
- Context: 用户要求钉钉附件链路只服务代码文本分析，图片继续走贴图问诊
- Instructions:
  - 钉钉附件上传支持纯文本代码文件，以及 `.doc`、`.docx` 这类代码文档附件。
  - 图片和 PDF 附件在入口直接返回固定提示，不进入上传、解析和索引链路。
  - 钉钉附件上传只支持单个文件；同一条消息里多个文件时，直接提示用户一次只发送 1 个文件。
  - 单个附件解析后的 `parsed.md` 文本在 `30KB` 以内时，直接全文注入给代码分析 skill；超过 `30KB` 再走检索片段注入。
  - 固定提示文案为“上传附件仅为代码文本文件服务，其他类型不支持。搜图问诊请直接贴图。”
